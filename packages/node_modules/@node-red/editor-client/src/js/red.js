...
var RED = (function() {
    ...
    function buildMainMenu() {
        ...
        menuOptions.push({id:"menu-item-view-menu",label:RED._("menu.label.view.view"),options:[
            {id:"menu-item-flow-drawer",label:"Show Flow Navigator",toggle:true,onselect:"core:toggle-flow-drawer", selected: false},
        ...
        console.log("Building main menu with options:", menuOptions.length, "items");
        RED.menu.init({id:"red-ui-header-button-sidemenu",options: menuOptions});

        // Add theme toggle button
        $('<li><a id="red-ui-header-button-theme-toggle" class="button" href="#" title="Toggle light/dark mode"><i class="fa fa-sun-o"></i></a></li>').appendTo(".red-ui-header-toolbar")
        
        // Theme switching logic
        function initThemeToggle() {
            const themeToggleBtn = $('#red-ui-header-button-theme-toggle');
            const body = $('body');
            
            // Get saved theme preference or default to light
            let currentTheme = localStorage.getItem('node-red-theme') || 'light';
            
            // Apply initial theme
            function applyTheme(theme) {
                if (theme === 'dark') {
                    body.attr('data-theme', 'dark');
                    themeToggleBtn.find('i').removeClass('fa-sun-o').addClass('fa-moon-o');
                    themeToggleBtn.attr('title', 'Switch to light mode');
                } else {
                    body.removeAttr('data-theme');
                    themeToggleBtn.find('i').removeClass('fa-moon-o').addClass('fa-sun-o');
            ...
            themeToggleBtn.on('click', function(e) {
                ...
                localStorage.setItem('node-red-theme', currentTheme);
                applyTheme(currentTheme);
                
                // Trigger a custom event for other components that might need to know about theme changes
                RED.events.emit('theme:changed', currentTheme);
            });
        }
    ...
    function loadEditor() {
        ...
        // Initialize deploy functionality (includes canvas deploy button)
        RED.deploy.init(RED.settings.theme("deployButton",null));

        // Initialize keyboard shortcuts (needed for copy/paste/delete functionality)
        RED.keyboard.init(function() {
        ...
        // Register flow drawer actions
        RED.actions.add("core:toggle-flow-drawer", function(state) {
            if (state === undefined) {
                RED.flowDrawer.toggle();
                RED.menu.toggleSelected("menu-item-flow-drawer");
            } else {
                if (state) {
                    RED.flowDrawer.show();
                } else {
                ...
                RED.menu.setSelected("menu-item-flow-drawer", state);
            }
        });

        // Note: Palette and sidebar actions are already registered by their respective modules
        // We don't need to register them again here

        $("#red-ui-main-container").show();

        loadPluginList();
        
        // Note: Interface color system removed - using project colors only
        
        // Initialize project colors when editor loads (fallback for when projects are enabled)
        if (RED.projects && RED.projects.settings) {
            // Check if there's a stored project color from session
            var storedColor = sessionStorage.getItem('current-project-color');
            if (storedColor) {
                RED.projects.settings.applyProjectColor(storedColor);
            }
        }
    }
    ...
    function buildEditor(options) {
        // Remove header creation - we don't want the navbar
        ...
        // Create main container without header offset
        ...
        // Set custom Node-Purple branding - removed since we don't have header
        // $('<span>').html("Node-Purple").appendTo(logo);

        $.getJSON(options.apiRootUrl+"theme", function(theme) {
            // Skip the default theme header configuration
            // We're using our custom Node-Purple branding instead
            if (theme.themes) {
