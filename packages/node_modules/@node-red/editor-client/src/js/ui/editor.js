...
RED.editor = (function() {
    ...
    function showEditDialogModal(node, defaultTab) {
        if (buildingEditDialog) { return }
        if (editStack.includes(node)) { return }
        buildingEditDialog = true;
        if (node.z && RED.workspaces.isLocked(node.z)) { return }
        ...
        $('.red-ui-editor-modal').remove();
        
        // Create modal dialog
        const modal = $(`
            <div class="red-ui-editor-modal">
                <div class="red-ui-editor-modal-content">
                    <div class="red-ui-editor-modal-header">
                        <h3>${RED._("editor.editNode",{type:node.type})}</h3>
                        <button class="red-ui-editor-modal-close">×</button>
                    </div>
                    <div class="red-ui-editor-modal-body">
                        <div class="red-ui-tray-body"></div>
                    </div>
                    <div class="red-ui-editor-modal-footer">
                        <div class="red-ui-tray-footer-left"></div>
                        <div class="red-ui-tray-footer-right">
                            <button id="node-dialog-cancel" class="red-ui-button">${RED._("common.label.cancel")}</button>
                            <button id="node-dialog-delete" class="red-ui-button red-ui-button-left">${RED._("common.label.delete")}</button>
                            <button id="node-dialog-ok" class="red-ui-button red-ui-button-primary">${RED._("common.label.done")}</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        // Setup modal content similar to tray
        const modalBody = modal.find('.red-ui-tray-body');
        const modalFooter = modal.find('.red-ui-editor-modal-footer');
        const modalFooterLeft = modal.find('.red-ui-tray-footer-left');
        
        if (editing_node.hasOwnProperty('outputs')) {
            editing_node.__outputs = editing_node.outputs;
        }

        modalBody.parent().css('overflow','hidden');

        var helpButton = $('<button type="button" class="red-ui-button"><i class="fa fa-book"></button>').on("click", function(evt) {
            evt.preventDefault();
            evt.stopPropagation();
            RED.sidebar.help.show(editing_node.type);
        }).appendTo(modalFooterLeft);
        RED.popover.tooltip(helpButton, RED._("sidebar.help.showHelp"));

        $('<input id="node-input-node-disabled" type="checkbox">').prop("checked",!!node.d).appendTo(modalFooterLeft).toggleButton({
            enabledIcon: "fa-circle-thin",
            disabledIcon: "fa-ban",
            invertState: true
        })
        ...
        prepareEditDialog(modalBody, nodeEditPanes, node, node._def, "node-input", defaultTab, function(_activeEditPanes) {
            activeEditPanes = _activeEditPanes;
            modalBody.i18n();
            modalFooter.i18n();
            buildingEditDialog = false;
            ...
            modal.find('#node-dialog-cancel').on('click', function() {
                closeModal();
            });
            
            modal.find('#node-dialog-delete').on('click', function() {
                var startingLength = editStack.length;
                var removedNodes = [];
                var removedLinks = [];
                var removedGroups = [];
                var startDirty = RED.nodes.dirty();
                if (editing_node._def.oneditdelete) {
                    try {
                        editing_node._def.oneditdelete.call(editing_node);
                    } catch(err) {
                        console.log("oneditdelete",editing_node.id,editing_node.type,err.toString());
                    }
                }
                ...
                if (editing_node.type.indexOf("subflow:") === 0) {
                    var subflow = RED.nodes.subflow(editing_node.type.substring(8));
                    removedNodes = [editing_node];
                    removedLinks = [];
                } else {
                    var result = RED.nodes.removeNode(editing_node.id);
                    removedNodes = result.nodes;
                    removedLinks = result.links;
                }
                historyEvent.nodes = removedNodes;
                historyEvent.links = removedLinks;
                historyEvent.groups = removedGroups;
            ...
            modal.find('#node-dialog-ok').on('click', function() {
                var editState = {
                    changes:{},
                    changed:false,
                    outputMap: null
                ...
                var isValid = validateNode(editing_node);

                var newDisabledState = $("#node-input-node-disabled").prop("checked");
                if (newDisabledState != editing_node.d) {
                    editState.changes.d = editing_node.d;
                    editing_node.d = newDisabledState;
                    editState.changed = true;
                }
                ...
                if (editing_node._def.defaults) {
                    for (d in editing_node._def.defaults) {
                        if (editing_node._def.defaults.hasOwnProperty(d)) {
                            ...
                            if (newValue != null && newValue !== editing_node[d]) {
                                if (d === "outputs") {
                                    editing_node.outputs = newValue;
                                    editState.outputMap = editing_node.__outputs;
                                    delete editing_node.__outputs;
                                    if (editing_node.outputs > editing_node.__outputs) {
                                        editState.outputMap = null;
                                    }
                                } else {
                                    editState.changes[d] = editing_node[d];
                ...
                if (editing_node._def.credentials) {
                    for (d in editing_node._def.credentials) {
                        if (editing_node._def.credentials.hasOwnProperty(d)) {
                            var input = $("#node-input-"+d);
                            newValue = input.val();
                            if (newValue != editing_node[d]) {
                                editState.changes[d] = editing_node[d];
                                editing_node[d] = newValue;
                                editState.changed = true;
                            }
                ...
                var removedLinks = updateNodeProperties(editing_node, editState.outputMap);
                if (removedLinks) {
                    editState.links = removedLinks;
                }

                RED.editor.validateNode(editing_node);
        ...
        function closeModal() {
            ...
            if (removeInfoEditorOnClose) {
                delete editing_node.infoEditor;
            } else if(editing_node.infoEditor__orig) {
                editing_node.infoEditor = editing_node.infoEditor__orig;
                delete editing_node.infoEditor__orig;
            }
        }
        ...
        modal.find('.red-ui-editor-modal-close').on('click', function() {
            closeModal();
        });
