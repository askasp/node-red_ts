...
RED.projects.settings = (function() {
    ...
    function createMainPane(activeProject) {
        ...
        var colorSection = $('<div class="red-ui-project-color-section" style="position: relative; margin: 20px 0;">').appendTo(pane);
        $('<h3>').text("Project Color").appendTo(colorSection);
        
        var colorRow = $('<div class="red-ui-project-color-row" style="display: flex; align-items: center; gap: 15px; margin-top: 10px;">').appendTo(colorSection);
        
        // Color preview circle showing current project color
        var colorPreview = $('<div class="red-ui-project-color-preview">')
            .css({
                'width': '32px',
                'height': '32px',
                'border-radius': '50%',
                ...
                'cursor': 'pointer',
                'background-color': activeProject.color || '#6b7280',
                'flex-shrink': '0',
        ...
        var colorLabel = $('<span>')
            .text('Choose a color to personalize this project workspace')
            .css({
        ...
        if (RED.user.hasPermission("projects.write")) {
            // Arc-style color palette (even darker variants)
            var arcColors = [
                '#b91c1c', '#c2410c', '#b45309', '#a16207', '#4d7c0f', '#15803d',
                '#047857', '#0f766e', '#0e7490', '#0369a1', '#1d4ed8', '#4338ca',
                '#6d28d9', '#7e22ce', '#a21caf', '#be185d', '#be123c', '#374151'
            ];

            colorPreview.on('click', function(e) {
                e.preventDefault();
                
                var pickerContainer = $('<div class="red-ui-project-color-picker-container">').css({
                    'padding': '16px',
                    'background': 'var(--red-ui-secondary-background)',
                    'border-radius': '8px',
                    'box-shadow': 'var(--red-ui-shadow)',
                    'min-width': '280px'
                });

                // Dark mode toggle section
                var themeRow = $('<div class="red-ui-project-theme-toggle">').css({
                    'display': 'flex',
                    'align-items': 'center',
                    'gap': '10px',
                    'margin-bottom': '16px',
                ...
                // Get current theme mode from project settings or default to light
                var currentTheme = (activeProject.themeMode) ? activeProject.themeMode : 'light';
                
                var themeCheckbox = $('<input type="checkbox">')
                    .prop('checked', currentTheme === 'dark')
                    .css({
                        'margin': '0'
                    })
                    .appendTo(themeLabel);

                $('<span>').text('Dark mode').appendTo(themeLabel);
                
                var picker = $('<div class="red-ui-arc-color-picker">').css({
                    'display': 'grid',
                    'grid-template-columns': 'repeat(6, 36px)',
                    'gap': '8px'
                }).appendTo(pickerContainer);
                
                arcColors.forEach(function(color) {
                    var colorOption = $('<button type="button">')
                        .css({
                            'width': '36px',
                            'height': '36px',
                            'border-radius': '50%',
                    ...
                    if (color === (activeProject.color || '#6b7280')) {
                        colorOption.css('border-color', '#ffffff');
                        $('<div>').css({
                            'position': 'absolute',
                            'top': '50%',
                            'left': '50%',
                    ...
                    colorOption.on('click', function(e) {
                        e.preventDefault();
                        var newColor = color;
                        var selectedTheme = themeCheckbox.is(':checked') ? 'dark' : 'light';
                        
                        // Update the preview
                        colorPreview.css('background-color', newColor);
                        
                        // Apply the theme immediately with the selected mode
                        applyProjectColor(newColor, selectedTheme);
                        
                        // Save both color and theme mode to project settings
                        saveProjectColorAndTheme(activeProject, newColor, selectedTheme);
                ...
                // Update theme when toggle changes (without changing color)
                themeCheckbox.on('change', function() {
                    var selectedTheme = $(this).is(':checked') ? 'dark' : 'light';
                    var currentColor = activeProject.color || '#6b7280';
                    
                    // Re-apply current color with new theme
                    applyProjectColor(currentColor, selectedTheme);
                    
                    // Save the theme mode preference
                    activeProject.themeMode = selectedTheme;
                    saveProjectColorAndTheme(activeProject, currentColor, selectedTheme);
                });
                
                var colorPanel = RED.popover.panel(pickerContainer);
                colorPanel.show({
                    target: colorPreview,
                    onclose: function() {
                        colorPreview.focus();
    ...
    // Helper function to adjust bright colors for better contrast
    function adjustColorForContrast(color) {
        var hsl = RED.utils.hexToHSL(color);
        
        // If the color is very bright (high saturation and lightness), darken it
        if (hsl.l > 70 && hsl.s > 40) {
            // Darken bright colors by reducing lightness
            var adjustedLightness = Math.max(35, hsl.l - 25); // Ensure minimum darkness
            
            // Convert back to hex
            var adjustedHsl = {h: hsl.h, s: hsl.s, l: adjustedLightness};
            return hslToHex(adjustedHsl);
        }
        
        return color; // Return original color if no adjustment needed
    }
    ...
    function hslToHex(hsl) {
        ...
        return '#'+'000000'.slice(0, 6-result.length)+result;
    }

    function applyProjectColor(color, theme) {
        if (!color) return;
        
        // Use theme parameter or get from current active project
        var activeProject = RED.projects.getActiveProject ? RED.projects.getActiveProject() : null;
        var themeMode = theme || (activeProject && activeProject.themeMode) || 'light';
        var isDark = themeMode === 'dark';
        
        // Apply to CSS custom properties immediately
        var root = document.documentElement;
        
        // Sidebar and palette colors (always use theme color)
        root.style.setProperty('--theme-sidebar-bg', color);
        root.style.setProperty('--theme-palette-bg', color);
        
        if (isDark) {
            // Dark mode: very dark canvas, slate gray nodes
            var canvasBg = '#0f1419';           // Very dark blue-gray
            var cardBg = '#1e293b';             // Slate gray for nodes
            var cardBorder = '#334155';        // Lighter slate for borders
            var cardHover = '#475569';         // Even lighter slate for hover
            var textColor = '#f8fafc';         // Very light text
            var secondaryText = '#cbd5e1';     // Light gray secondary text
            var nodeText = '#f1f5f9';          // Slightly warm white for nodes
            var borderColor = RED.utils.getDarkerColor(color);
            ...
            root.style.setProperty('--theme-node-bg', cardBg);
            root.style.setProperty('--theme-node-text', nodeText);
            root.style.setProperty('--theme-node-text-secondary', secondaryText);
            root.style.setProperty('--theme-sidebar-text', textColor);
            root.style.setProperty('--theme-palette-text', textColor);
            root.style.setProperty('--theme-sidebar-border', borderColor);
            root.style.setProperty('--theme-palette-border', borderColor);
            
            // Set grid dots to be visible on dark background
            root.style.setProperty('--theme-canvas-dot', 'rgba(255, 255, 255, 0.15)');
            
        } else {
            // Light mode: very light theme-tinted canvas, white nodes
            var lightColor = RED.utils.getLighterColor(color, 0.95);
            var textColor = '#374151';
            var borderColor = RED.utils.getDarkerColor(color);
            
            root.style.setProperty('--theme-canvas-bg', lightColor);
            root.style.setProperty('--theme-card-bg', '#ffffff');
            root.style.setProperty('--theme-card-border', '#e2e8f0');
            root.style.setProperty('--theme-card-hover', RED.utils.getLighterColor(color, 0.05));
            root.style.setProperty('--theme-node-bg', '#ffffff');
            root.style.setProperty('--theme-node-text', textColor);
            root.style.setProperty('--theme-node-text-secondary', '#64748b');
            root.style.setProperty('--theme-sidebar-text', textColor);
            root.style.setProperty('--theme-palette-text', textColor);
            root.style.setProperty('--theme-sidebar-border', borderColor);
            root.style.setProperty('--theme-palette-border', borderColor);
            
            // Set grid dots to be visible on light background
            root.style.setProperty('--theme-canvas-dot', 'rgba(0, 0, 0, 0.1)');
        }
        
        // Accent color (always theme color regardless of mode)
        root.style.setProperty('--theme-accent', color);
        
        // Create hover and active variants of the accent color
        var hoverColor = RED.utils.getDarkerColor(color, 0.15); // Slightly darker for hover
        var activeColor = RED.utils.getDarkerColor(color, 0.25); // Even darker for active
        root.style.setProperty('--theme-accent-hover', hoverColor);
        root.style.setProperty('--theme-accent-active', activeColor);
        
        // Store the current project color and theme in session storage for immediate access
        sessionStorage.setItem('current-project-color', color);
        sessionStorage.setItem('current-project-theme', themeMode);
        
        console.log('Applied project color:', color, 'with theme:', themeMode);
        
        // Emit event for other components that might need to update
        RED.events.emit('project:color-changed', { 
            color: color, 
            theme: themeMode,
            lightColor: isDark ? '#0f1419' : lightColor,
            textColor: textColor,
            isDark: isDark
        });
    }
    
    function saveProjectColorAndTheme(activeProject, color, theme) {
        if (!activeProject || !RED.user.hasPermission("projects.write")) {
            return;
        }
        ...
        utils.sendRequest({
            url: "projects/" + encodeURIComponent(activeProject.name),
            type: "PUT",
            data: projectData
        }).then(function() {
            // Update local project reference
            activeProject.color = color;
            activeProject.themeMode = theme;
            RED.notify("Project color and theme updated", "success");
        }).catch(function(err) {
            RED.notify("Failed to save project color: " + (err.responseText || err.message || "Unknown error"), "error");
            console.error("Error saving project color:", err);
        });
    }
    
    // Keep the old function for backward compatibility, but make it use the new one
    function saveProjectColor(activeProject, color) {
        var theme = (activeProject && activeProject.themeMode) || 'light';
        saveProjectColorAndTheme(activeProject, color, theme);
    }
    ...
    function init(_utils) {
        ...
        // Listen for project load/switch events to apply colors
        RED.events.on('projects:load', function(project) {
            console.log('Project loaded - applying color:', project ? project.color : 'none');
            loadProjectColor(project);
        });
        
        // Apply color when project changes
        RED.events.on('projects:change', function(project) {
            console.log('Project changed - applying color:', project ? project.color : 'none');
            loadProjectColor(project);
        });
    }
    return {
        ...
        loadProjectColor: loadProjectColor
    };

    function loadProjectColor(activeProject) {
        console.log('Loading project color for:', activeProject ? activeProject.name : 'no project', 'Color:', activeProject ? activeProject.color : 'none', 'Theme:', activeProject ? activeProject.themeMode : 'none');
        
        if (activeProject && activeProject.color) {
            var theme = activeProject.themeMode || 'light';
            applyProjectColor(activeProject.color, theme);
            console.log('Applied project color:', activeProject.color, 'with theme:', theme);
        } else {
            // Apply default color with light theme
            applyProjectColor('#6b7280', 'light');
            console.log('Applied default color: #6b7280 with light theme');
        }
    }
})();
