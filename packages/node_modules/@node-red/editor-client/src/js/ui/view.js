...
RED.view = (function() {
    var space_width = 8000,
        ...
        lineCurveScale = 0.75,
        scaleFactor = 1,
        node_width = 150, // Minimum node width
        node_height = 100, // Reduced by 10px from original 120
    ...
    function init() {
        ...
        eventLayer = outer
            ...
            .on("touchmove", function(){
                    ...
                    if (d3.event.touches.length<2) {
                        if (touchStartTime && d3.event.touches && d3.event.touches.length > 0) {
                            touch0 = d3.event.touches.item(0);
        ...
        // Function to update pattern colors based on theme and project colors
        function updateDotPattern() {
            // Get the current canvas background color from CSS variables
            const computedStyle = getComputedStyle(document.documentElement);
            let bgColor = computedStyle.getPropertyValue('--theme-canvas-bg').trim();
            
            // Fallback to default theme colors if no project color is set
            if (!bgColor) {
                const isDark = document.body.getAttribute('data-theme') === 'dark';
                bgColor = isDark ? '#111827' : '#f8fafc';
            }
            
            // Calculate appropriate dot color based on background
            // Use a slightly darker/lighter version for good contrast
            let dotColor;
            if (bgColor.startsWith('#')) {
                ...
                // Choose dot color based on background luminance
                if (luminance > 0.8) {
                    // Very light background - use darker dots
                    dotColor = `rgba(${Math.max(0, r - 60)}, ${Math.max(0, g - 60)}, ${Math.max(0, b - 60)}, 0.4)`;
                } else if (luminance > 0.5) {
                    // Medium light background - use medium dark dots
                    dotColor = `rgba(${Math.max(0, r - 40)}, ${Math.max(0, g - 40)}, ${Math.max(0, b - 40)}, 0.5)`;
                } else {
                    // Dark background - use lighter dots
                    dotColor = `rgba(${Math.min(255, r + 40)}, ${Math.min(255, g + 40)}, ${Math.min(255, b + 40)}, 0.4)`;
                }
            } else {
            ...
            pattern.select("rect").attr("fill", bgColor);
            pattern.select("circle").attr("fill", dotColor);
        }
        
        // First add background rectangle for the pattern
        pattern.append("rect")
            .attr("width", gridSize)
            .attr("height", gridSize);
        ...
        // Update pattern initially
        updateDotPattern();
        
        // Listen for project color changes (single event, no flashing)
        RED.events.on('project:color-changed', updateDotPattern);

        // Workspace Background with dot pattern
            ...
            drop: function( event, ui ) {
                ...
                try {
                    ...
                    try {
                        ...
                        var label = RED.utils.getNodeLabel(nn, "");
                        
                        // Calculate dynamic dimensions
                        var dimensions = calculateNodeDimensions(nn);
                        nn.w = dimensions[0];
                        nn.h = dimensions[1];
                    } catch(err) {
                        // Fallback on error
                        nn.h = 40; // Reduced by 10px
                        nn.w = node_width; // Use minimum width as fallback
                    }
    ...
    function canvasMouseDown() {
        ...
        if (d3.event.touches || d3.event.button === 0) {
            if (
                ...
                // CTRL not being held - start lasso selection
                if (!d3.event.altKey && (!d3.event.touches || d3.event.touches.length === 0)) {
                    // ALT not held, not a multi-touch event - Trigger lasso selection
                    const point = d3.mouse(this);
                    
                    // Ensure eventLayer exists
                    if (eventLayer) {
                        try {
                            ...
                            console.error("Error creating lasso:", err);
                        }
                    }
                }
    ...
    function showQuickAddDialog(options) {
        ...
        console.log("About to call RED.typeSearch.show");
        
        if (!RED.typeSearch || typeof RED.typeSearch.show !== 'function') {
            console.error("RED.typeSearch.show is not available", RED.typeSearch);
            // Fallback - hide the ghost node and reset state
            if (ghostNode) {
                ghostNode.remove();
            }
            resetMouseVars();
        ...
        try {
            RED.typeSearch.show({
                ...
                cancel: function() {
                    console.log("typeSearch cancelled");
                    if (quickAddLink) {
                        if (quickAddLink.el) {
            ...
            console.error("Error in RED.typeSearch.show:", error);
            // Fallback - hide the ghost node and reset state
            if (ghostNode) {
                ghostNode.remove();
            }
    ...
    function getLabelParts(str, className, maxWidth) {
        // Enhanced empty label detection
        if (!str || 
            str.trim() === "" || 
            str === "undefined" || 
            str === "null" ||
        ...
        maxWidth = maxWidth || (node_width - 16); // Default to node width minus padding
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            // Check if line fits in one go
            var lineWidth = calculateTextDimensions(line, className)[0];
            if (lineWidth <= maxWidth) {
                ...
                for (var j = 0; j < words.length; j++) {
                    ...
                    var testWidth = calculateTextDimensions(testLine, className)[0];
                    ...
                    if (testWidth <= maxWidth) {
                        ...
                        if (currentLine) {
                            ...
                            maxLineWidth = Math.max(maxLineWidth, calculateTextDimensions(currentLine, className)[0]);
                        }
                        
                        // Check if single word is too long
                        var wordWidth = calculateTextDimensions(word, className)[0];
                        if (wordWidth > maxWidth) {
                            // Break the word
                            var chars = word.split('');
                            var partialWord = "";
                            for (var k = 0; k < chars.length; k++) {
                                var testChar = partialWord + chars[k];
                                var testCharWidth = calculateTextDimensions(testChar, className)[0];
                                if (testCharWidth <= maxWidth - 10) { // Leave some margin
                                    partialWord = testChar;
                                } else {
                                    if (partialWord) {
                                        wrappedLines.push(partialWord);
                                        maxLineWidth = Math.max(maxLineWidth, calculateTextDimensions(partialWord, className)[0]);
                                    }
                                    partialWord = chars[k];
                                }
                            }
        ...
        return {
            lines: wrappedLines,
            width: maxLineWidth,
            isEmpty: wrappedLines.length === 0 || (wrappedLines.length === 1 && wrappedLines[0] === "")
        };
    }
            ...
            if (typeof TouchEvent != "undefined" && evt instanceof TouchEvent) {
                ...
                for (let nodeIdx = 0; nodeIdx < activeNodes.length; nodeIdx++) {
                    ...
                    if (RED.view.tools.isPointInNode(n, mouse_position)) {
                        found = true;
                        mouseup_node = n;
                        // portType = mouseup_node.type === 'junction' ? direction : mouseup_node.inputs > 0 ? PORT_TYPE_INPUT : PORT_TYPE_OUTPUT;
                        portType = direction;
                        portIndex = 0;
                        break
                    }
    ...
    function redrawStatus(d,nodeEl) {
        ...
        if (nodeEl) {
            // Status text is always hidden - status is shown via the status dot only
            nodeEl.__statusGroup__.style.display = "none";
            
            // Update status dot color when status changes
            if (nodeEl.__statusDot__) {
                var statusColor = "#9ca3af"; // Default gray for no status
                if (d.d === true) {
                    statusColor = "#6b7280"; // Disabled - darker gray
                } else if (d._def && d._def.category === "config") {
                    statusColor = "#3b82f6"; // Config nodes - blue
                } else if (d.status) {
                    // Use node's actual status
                    switch(d.status.fill) {
                        case "red": statusColor = "#ef4444"; break;
                        case "green": statusColor = "#22c55e"; break;
                        case "yellow": statusColor = "#f59e0b"; break;
                        case "blue": statusColor = "#3b82f6"; break;
                        case "grey":
                        case "gray": statusColor = "#6b7280"; break;
                        default: statusColor = "#22c55e"; // Default green for active
                    }
                } else {
                    statusColor = "#22c55e"; // Default green for healthy/ready
                }
                nodeEl.__statusDot__.setAttribute("fill", statusColor);
            }
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            if (activeSubflow) {
                ...
                outGroup.each(function(d,i) {
                    ...
                    // Port label color will be handled by CSS theme
                ...
                subflowOutputs.each(function(d,i) {
                    if (d.dirty) {
                        ...
                        if (d.resize || this.__hideLabel__ !== hideLabel || this.__label__ !== label) {
                            ...
                            // Apply minimal height to subflow ports based on content
                            // Much more compact design 
                            if (hideLabel) {
                                d.h = 50; // Minimal compact height
                            } else {
                                d.h = Math.max(50, 35 + 16*labelParts.lines.length + 10); // Badge area + text + minimal padding
                            }
                        ...
                        if (d.resize) {
                            var ow = d.w;
                            // Apply dynamic width to subflow ports
                            var dimensions = calculateNodeDimensions(d);
                            d.w = dimensions[0];
                            if (ow !== undefined) {
                                d.x += (d.w-ow)/2;
                            }
            ...
            nodeEnter.each(function(d,i) {
                ...
                if (d._def.button) {
                    ...
                    buttonGroup.setAttribute("data-node-type", d.type); // Add node type for styling
                    ...
                    // Create small, elegant buttons
                    var bgButton;
                    if (d.type === "inject" || d.type === "debug") {
                        // Create small circular button for inject and debug nodes
                        bgButton = document.createElementNS("http://www.w3.org/2000/svg","circle");
                        bgButton.__data__ = d;
                        bgButton.setAttribute("class","red-ui-flow-node-button-button");
                        bgButton.setAttribute("cx", d._def.align == "right"? 19:7);
                        bgButton.setAttribute("cy", node_height/2);
                        bgButton.setAttribute("r", 8); // Smaller radius for elegance
                        
                        // Create icon text element
                        var iconText = document.createElementNS("http://www.w3.org/2000/svg","text");
                        iconText.setAttribute("class","red-ui-flow-node-button-icon");
                        iconText.setAttribute("x", d._def.align == "right"? 19:7);
                        iconText.setAttribute("y", node_height/2);
                        
                        if (d.type === "inject") {
                            iconText.textContent = "\uf04b"; // FontAwesome play icon - small and elegant
                        } else if (d.type === "debug") {
                            iconText.textContent = "\uf06e"; // FontAwesome eye icon - small and elegant
                        }
                        
                        buttonGroup.appendChild(iconText);
                        node[0][0].__buttonGroupIcon__ = iconText;
                    } else {
                        // Keep original rectangular button for other nodes
                        bgButton = document.createElementNS("http://www.w3.org/2000/svg","rect");
                        bgButton.__data__ = d;
                        bgButton.setAttribute("class","red-ui-flow-node-button-button");
                    ...
                    d3.select(bgButton)
                        .on("mousedown",function(d) {if (!lasso && isButtonEnabled(d)) {focusView();d3.event.preventDefault(); d3.event.stopPropagation();}})
                        .on("mouseup",function(d) {if (!lasso && isButtonEnabled(d)) {d3.event.preventDefault();d3.event.stopPropagation();}})
                        .on("mouseover",function(d) {if (!lasso && isButtonEnabled(d)) { /* hover handled by CSS */ }})
                        .on("mouseout",function(d) {if (!lasso && isButtonEnabled(d)) { /* hover handled by CSS */ }})
                ...
                // ===== MULTI-SECTION SVG CARD =====
                // Create a modern card with multiple sections
                
                // 1. CARD SHADOW (for elevation)
                // 1. DAISYUI-STYLE ELEVATION SHADOW (multiple layers like shadow-md)
                var shadowDefs = document.createElementNS("http://www.w3.org/2000/svg","defs");
                var filter = document.createElementNS("http://www.w3.org/2000/svg","filter");
                filter.setAttribute("id", "shadow-" + d.id);
                filter.setAttribute("x", "-20%");
                filter.setAttribute("y", "-20%");
                filter.setAttribute("width", "140%");
                filter.setAttribute("height", "140%");
                
                // First shadow layer (like 0 4px 6px rgba(0,0,0,0.07))
                var feDropShadow1 = document.createElementNS("http://www.w3.org/2000/svg","feDropShadow");
                feDropShadow1.setAttribute("dx", "0");
                feDropShadow1.setAttribute("dy", "4");
                feDropShadow1.setAttribute("stdDeviation", "3");
                feDropShadow1.setAttribute("flood-color", "rgba(0,0,0,0.07)");
                filter.appendChild(feDropShadow1);
                
                // Second shadow layer (like 0 2px 4px rgba(0,0,0,0.06))
                var feDropShadow2 = document.createElementNS("http://www.w3.org/2000/svg","feDropShadow");
                feDropShadow2.setAttribute("dx", "0");
                feDropShadow2.setAttribute("dy", "2");
                feDropShadow2.setAttribute("stdDeviation", "2");
                feDropShadow2.setAttribute("flood-color", "rgba(0,0,0,0.06)");
                filter.appendChild(feDropShadow2);
                
                shadowDefs.appendChild(filter);
                nodeContents.appendChild(shadowDefs);
                node[0][0].__shadowFilter__ = filter;

                // 1.5. CREATE PORT GROUPS (before main background so they appear behind)
                // Input port group
                var inputPortGroup = document.createElementNS("http://www.w3.org/2000/svg","g");
                inputPortGroup.setAttribute("class","red-ui-flow-port-input");
                inputPortGroup.style.display = "none"; // Initially hidden
                nodeContents.appendChild(inputPortGroup);
                node[0][0].__inputPortGroup__ = inputPortGroup;

                // Output port groups container  
                var outputPortsContainer = document.createElementNS("http://www.w3.org/2000/svg","g");
                outputPortsContainer.setAttribute("class","red-ui-flow-ports-output");
                nodeContents.appendChild(outputPortsContainer);
                node[0][0].__outputPortsContainer__ = outputPortsContainer;

                // 2. MAIN CARD BACKGROUND (with DaisyUI shadow filter)
                var mainRect = document.createElementNS("http://www.w3.org/2000/svg","rect");
                mainRect.__data__ = d;
                ...
                mainRect.setAttribute("filter", "url(#shadow-" + d.id + ")");
                node[0][0].__mainRect__ = mainRect;
                d3.select(mainRect)
                    .on("mouseup",nodeMouseUp)
                    .on("mousedown",nodeMouseDown)
                ...
                // 3. NODE TYPE BADGE (small badge at top like in the image)
                function getNodeBadgeInfo(nodeType) {
                    var badges = {
                        // Network & HTTP
                        'http request': { text: 'HTTP Request', color: '#059669', icon: '○' },  // emerald-600
                        'http in': { text: 'HTTP In', color: '#0285c7', icon: '◄' },          // blue-600
                        'api-endpoint': { text: 'API Endpoint', color: '#0285c7', icon: '◄' },          // blue-600
                        'http response': { text: 'HTTP Response', color: "#0285c7", icon: '►' }, // violet-600
                        'websocket in': { text: 'WebSocket In', color: '#0891b2', icon: '⟲' }, // cyan-600
                        'websocket out': { text: 'WebSocket Out', color: '#0891b2', icon: '⟲' },
                        'mqtt in': { text: 'MQTT In', color: '#dc2626', icon: '◉' },         // red-600
                        'mqtt out': { text: 'MQTT Out', color: '#dc2626', icon: '◉' },
                        'tcp in': { text: 'TCP In', color: '#4338ca', icon: '◊' },          // indigo-600
                        'tcp out': { text: 'TCP Out', color: '#4338ca', icon: '◊' },
                        'udp in': { text: 'UDP In', color: '#9333ea', icon: '◎' },          // purple-600
                        'udp out': { text: 'UDP Out', color: '#9333ea', icon: '◎' },
                        
                        // Functions & Logic
                        'function': { text: 'Function', color: '#1e40af', icon: '⚡' },       // blue-800 (TypeScript dark blue)
                        'typescript-function': { text: 'TypeScript', color: '#1e40af', icon: '⟅' }, // blue-800
                        'typescript-file': { text: 'TS File', color: '#3178c6', icon: '📁' }, // TypeScript blue
                        'switch': { text: 'Switch', color: '#ca8a04', icon: '⌥' },          // yellow-600
                        'change': { text: 'Change', color: '#0891b2', icon: '⟐' },          // cyan-600
                        'range': { text: 'Range', color: '#65a30d', icon: '―' },           // lime-600
                        'split': { text: 'Split', color: '#c2410c', icon: '⊗' },           // orange-700
                        'join': { text: 'Join', color: '#c2410c', icon: '⊕' },             // orange-700
                        'sort': { text: 'Sort', color: '#7c2d12', icon: '↕' },             // orange-800
                        'batch': { text: 'Batch', color: '#059669', icon: '⊞' },           // emerald-600
                        
                        // I/O & Triggers  
                        'inject': { text: 'Inject', color: '#dc2626', icon: '⬇' },         // red-600
                        'debug': { text: 'Debug', color: '#ea580c', icon: '◐' },          // orange-600
                        'complete': { text: 'Complete', color: '#16a34a', icon: '✓' },     // green-600
                        'catch': { text: 'Catch', color: '#dc2626', icon: '⚠' },          // red-600
                        'status': { text: 'Status', color: '#0891b2', icon: '◈' },        // cyan-600
                        'link in': { text: 'Link In', color: '#6b7280', icon: '◄' },      // gray-500
                        'link out': { text: 'Link Out', color: '#6b7280', icon: '►' },
                        
                        // Templates & Data
                        'template': { text: 'Template', color: '#9333ea', icon: '◓' },     // purple-600
                        'csv': { text: 'CSV', color: '#059669', icon: '⋮' },              // emerald-600
                        'html': { text: 'HTML', color: '#ea580c', icon: '◐' },           // orange-600
                        'json': { text: 'JSON', color: '#0891b2', icon: '{}' },           // cyan-600
                        'xml': { text: 'XML', color: '#7c3aed', icon: '<>' },             // violet-600
                        'yaml': { text: 'YAML', color: '#7c2d12', icon: '≡' },           // orange-800
                        
                        // Storage & Databases
                        'file': { text: 'File', color: '#374151', icon: '◯' },            // gray-700
                        'file in': { text: 'File In', color: '#374151', icon: '◯' },
                        'watch': { text: 'Watch', color: '#374151', icon: '◉' },         // gray-700
                        'mongodb': { text: 'MongoDB', color: '#059669', icon: '⟐' },      // emerald-600 (MongoDB green)
                        'redis': { text: 'Redis', color: '#dc2626', icon: '◈' },         // red-600 (Redis red)
                        'redis-stream-ack': { text: 'Redis Stream Ack', color: '#dc2626', icon: '◈' }, // red-600 (Redis red)
                        'redis-stream-worker': { text: 'Redis Stream Worker', color: '#dc2626', icon: '◈' }, // red-600 (Redis red)
                        'mysql': { text: 'MySQL', color: '#ea580c', icon: '◐' },         // orange-600
                        'postgres': { text: 'PostgreSQL', color: '#2563eb', icon: '◑' }, // blue-600
                        'sqlite': { text: 'SQLite', color: '#6b7280', icon: '◒' },       // gray-500
                        'redis-mongodb-changestream': { text: 'MongoDB → Redis', color: '#059669', icon: '↔' }, // emerald-600
                        
                        // Time & Delays
                        'delay': { text: 'Delay', color: '#ca8a04', icon: '◷' },          // yellow-600
                        'trigger': { text: 'Trigger', color: '#dc2626', icon: '◯' },      // red-600
                        'rbe': { text: 'Filter (RBE)', color: '#0891b2', icon: '⊙' },    // cyan-600
                        
                        // Comments & Organization
                        'comment': { text: 'Comment', color: '#6b7280', icon: '◐' },     // gray-500
                        'group': { text: 'Group', color: '#374151', icon: '◧' },         // gray-700
                        'subflow': { text: 'Subflow', color: '#7c3aed', icon: '⟐' },     // violet-600
                        
                        // Unknown/Default
                        'unknown': { text: 'Unknown', color: '#6b7280', icon: '?' }      // gray-500
                    };
                    var cleanType = nodeType.toLowerCase().replace(/\s+/g, '-');
                    return badges[cleanType] || { text: nodeType, color: '#6b7280', icon: '⚙️' };
                }

                var badgeInfo = getNodeBadgeInfo(d.type);
                
                // Icon background (larger colored square) - vertically centered
                var iconRect = document.createElementNS("http://www.w3.org/2000/svg","rect");
                iconRect.__data__ = d;
                iconRect.setAttribute("class", "red-ui-flow-node-icon-bg");
                iconRect.setAttribute("x", 8);
                iconRect.setAttribute("y", (d.h - 84) / 2); // Properly centered for reduced node height
                iconRect.setAttribute("width", 48);
                iconRect.setAttribute("height", 48);
                iconRect.setAttribute("rx", 6);
                iconRect.setAttribute("ry", 6);
                iconRect.setAttribute("fill", badgeInfo.color);
                iconRect.setAttribute("stroke", "none");
                iconRect.style["pointer-events"] = "none";
                iconRect.style["fill"] = badgeInfo.color;
                node[0][0].__iconRect__ = iconRect;
                nodeContents.appendChild(iconRect);

                // Badge icon (with Lucide icon support)
                function createLucideIcon(nodeType, x, y) {
                    // Lucide SVG path data for each icon
                    var lucideIcons = {
                        'globe': 'M22 12A10 10 0 0 0 12 2v10l10 0z M22 12A10 10 0 1 1 12 2',
                        'arrow-down-circle': 'M22 12A10 10 0 1 1 12 2a10 10 0 0 1 10 10z M8 12l4 4 4-4 M12 8v8',
                        'arrow-up-circle': 'M22 12A10 10 0 1 1 12 2a10 10 0 0 1 10 10z M16 12l-4-4-4 4 M12 17h0',
                        'zap': 'M13 2L3 14h9l-1 8 10-12h-9l1-8z',
                        'code': 'M16 18l6-6-6-6 M8 6l-6 6 6 6',
                        'bug': 'M8 6c0-1.11.89-2 2-2h4c1.11 0 2 .89 2 2v2l3 1v8l-3 1v1c0 1.11-.89 2-2 2h-4c-1.11 0-2-.89-2-2v-1l-3-1V9l3-1V6z M9 10h6 M9 14h6 M7 8L5 6 M7 16L5 18 M17 8l2-2 M17 16l2 2',
                        'play-circle': 'M22 12A10 10 0 1 1 12 2a10 10 0 0 1 10 10z M10 8l6 4-6 4V8z',
                        'check-circle': 'M22 12A10 10 0 1 1 12 2a10 10 0 0 1 10 10z M9 12l2 2 4-4',
                        'alert-triangle': 'M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z M12 9v4 M12 17h0',
                        ...
                        'file-code': 'M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z M14 2v6h6 M10 12l-2 2 2 2 M14 14l2-2-2-2',
                        'database': 'M12 8c-4.42 0-8-1.79-8-4s3.58-4 8-4 8 1.79 8 4-3.58 4-8 4z M4 4v8c0 2.21 3.58 4 8 4s8-1.79 8-4V4 M4 12v8c0 2.21 3.58 4 8 4s8-1.79 8-4v-8',
                        'hard-drive': 'M4 6h16v12H4V6z M2 9h20 M2 15h20 M6 12h0 M10 12h0',
                        'clock': 'M12 2a10 10 0 1 0 10 10A10 10 0 0 0 12 2z M12 6v6l4 2',
                        'git-branch': 'M6 3a3 3 0 1 0 0 6 3 3 0 0 0 0-6z M18 15a3 3 0 1 0 0 6 3 3 0 0 0 0-6z M8.59 13.51l4.82 4.82 M15.41 10.49l-4.82-4.82',
                        'edit-3': 'M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7 M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z',
                        'scissors': 'M9 6l6 6-6 6 M21 3l-6 6 M3 3l18 18 M9 18l6-6 6 6',
                        'link': 'M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71 M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71',
                        'radio': 'M4.9 19.1C1 15.2 1 8.8 4.9 4.9 M19.1 4.9c3.9 3.9 3.9 10.3 0 14.2 M8.5 16.5c-1.6-1.6-1.6-4.2 0-5.8 M15.5 7.5c1.6 1.6 1.6 4.2 0 5.8 M12 12h0',
                        'circle': 'M22 12A10 10 0 1 1 12 2a10 10 0 0 1 10 10z'
                    };
                    
                    // Map node types to icon names
                    var iconMap = {
                        'http request': 'globe',
                        'http in': 'arrow-down-circle',
                        'api-endpoint': 'arrow-down-circle',
                        'http response': 'arrow-up-circle',
                        'function': 'zap',
                        'typescript-function': 'code',
                        'typescript-file': 'file-code',
                        'debug': 'bug',
                        'inject': 'play-circle',
                        'complete': 'check-circle',
                        'catch': 'alert-triangle',
                        'template': 'file-text',
                        'json': 'braces',
                        'csv': 'list',
                        'file': 'file',
                        'mongodb': 'database',
                        'redis-mongodb-changestream': 'database',
                        'redis': 'hard-drive',
                        'redis-stream-ack': 'hard-drive',
                        'redis-stream-worker': 'hard-drive',
                        'delay': 'clock',
                        'switch': 'git-branch',
                        'change': 'edit-3',
                        'split': 'scissors',
                    ...
                    var iconName = iconMap[nodeType] || 'circle';
                    var pathData = lucideIcons[iconName];
                    
                    // Create SVG icon element
                    var iconSvg = document.createElementNS("http://www.w3.org/2000/svg","svg");
                    iconSvg.setAttribute("width", "64");
                    iconSvg.setAttribute("height", "64"); 
                    iconSvg.setAttribute("x", x + 4);
                    iconSvg.setAttribute("y", y -10);
                    ...
                    iconSvg.style["pointer-events"] = "none";
                    
                    // Add the path element with the icon data
                ...
                // Node icon (Lucide icons) - centered in the icon rectangle and vertically centered
                var iconY = (d.h - 48) / 2;
                var nodeIcon = createLucideIcon(d.type, 16, iconY + 4); // Centered within the icon rect
                nodeIcon.__data__ = d;
                //nodeIcon.setAttribute("class", "red-ui-flow-node-icon");
                nodeIcon.style["pointer-events"] = "none";
                node[0][0].__nodeIcon__ = nodeIcon;
                nodeContents.appendChild(nodeIcon);

                // Badge text - hidden in new design
                var badgeText = document.createElementNS("http://www.w3.org/2000/svg", "text");
                badgeText.__data__ = d;
                badgeText.setAttribute("x",12);
                badgeText.setAttribute("y", 12);
                badgeText.setAttribute("font-family", "Inter, 'Segoe UI', system-ui, sans-serif");
                badgeText.setAttribute("font-size", "12");
                badgeText.setAttribute("font-weight", "400");
                badgeText.setAttribute("fill", "#ffffff");
                badgeText.setAttribute("stroke", "#ffffff");
                badgeText.setAttribute("letter-spacing", "0.01em");
                badgeText.setAttribute("dominant-baseline", "middle");
                badgeText.style["text-rendering"] = "optimizeLegibility";
                badgeText.style["-webkit-font-smoothing"] = "antialiased";
                badgeText.style["-moz-osx-font-smoothing"] = "grayscale";
                badgeText.style["pointer-events"] = "none";
                badgeText.style["fill"] = "#ffffff"; // Ensure text stays white
                badgeText.style["stroke"] = "#ffffff";
                badgeText.style["display"] = "none"; // Hide badge text in new design
                badgeText.textContent = badgeInfo.text;
                node[0][0].__badgeText__ = badgeText;
                nodeContents.appendChild(badgeText);


                // 4. STATUS INDICATOR (top right - reflects actual node status)
                var statusDot = document.createElementNS("http://www.w3.org/2000/svg","circle");
                statusDot.__data__ = d;
                statusDot.setAttribute("class", "red-ui-flow-node-status-dot");
                statusDot.setAttribute("cx", d.w - 12);
                statusDot.setAttribute("cy", 12);
                statusDot.setAttribute("r", 3);
                // Dynamic color based on node status
                var statusColor = "#9ca3af"; // Default gray for no status
                if (d.d === true) {
                    statusColor = "#6b7280"; // Disabled - darker gray
                } else if (d._def && d._def.category === "config") {
                ...
                statusDot.style["pointer-events"] = "none";
                node[0][0].__statusDot__ = statusDot;
                nodeContents.appendChild(statusDot);
                ...
                if (d._def.icon) {
                    ...
                    // Hide icon to avoid conflicts with our badge design
                    icon_groupEl.style["display"] = "none";
                    ...
                    // Skip icon border to avoid visual artifacts in card design
                ...
                // 5. NODE TYPE TEXT (first row - uppercase service type)
                var typeText = document.createElementNS("http://www.w3.org/2000/svg","g");
                typeText.setAttribute("class","red-ui-flow-node-type-label");
                typeText.setAttribute("transform","translate(64, 26)"); // After larger icon, positioned lower
                typeText.setAttribute("y","26"); // After larger icon, positioned lower
                nodeContents.appendChild(typeText);
                node[0][0].__typeTextGroup__ = typeText;

                // 6. NODE NAME TEXT (second row - node name/description)
                var text = document.createElementNS("http://www.w3.org/2000/svg","g");
                text.setAttribute("class","red-ui-flow-node-label"+(hideLabel?" hide":""));
                text.setAttribute("transform","translate(64, 42)"); // Below the type text, after larger icon with proper spacing
                nodeContents.appendChild(text);
                node[0][0].__textGroup__ = text;

                // Status text removed - status is now shown via the status dot only
                ...
                statusEl.style.display = "none"; // Hide status text entirely
            ...
            node.each(function(d,i) {
                ...
                if (d.dirty) {
                    ...
                    if (d.resize || this.__hideLabel__ !== hideLabel || this.__label__ !== label || this.__outputs__.length !== d.outputs) {
                        ...
                        var baseHeight = 22; // Reduced from 32 to 22 (10px less)
                        
                        if (hideLabel || labelParts.isEmpty) {
                            // No label needed - just show icon and type text
                            d.h = baseHeight + 8; // Icon + minimal padding
                        } else {
                            // Has label - add space for icon, type text and name text with proper line height
                            var lineHeight = 16; // Spacing between lines
                            var textPadding = 8; // Reduced padding for horizontal layout
                            d.h = baseHeight + textPadding + 16 + (labelParts.lines.length * lineHeight); // Icon + type + name
                        }
                        ...
                        // Make sure labelParts is available for text positioning even when not resizing
                        labelParts = getLabelParts(label, "red-ui-flow-node-label", d.w - 16);
                    }

                    if (d.resize) {
                        var ow = d.w;
                        // Apply dynamic width to all nodes
                        var dimensions = calculateNodeDimensions(d);
                        d.w = dimensions[0];
                        d.h = dimensions[1];
                        if (ow !== undefined) {
                    ...
                    if (mouse_mode != RED.state.MOVING_ACTIVE) {
                        this.classList.toggle("red-ui-flow-node-disabled", d.d === true);
                        // Update main card dimensions
                        this.__mainRect__.setAttribute("width", d.w)
                        this.__mainRect__.setAttribute("height", d.h)
                        this.__mainRect__.classList.toggle("red-ui-flow-node-highlighted",!!d.highlighted );

                        // DaisyUI shadows are handled by SVG filter - no manual dimension updates needed

                        // Update badge width if needed (dynamic based on text length)
                        if (this.__badgeRect__ && this.__badgeText__) {
                            var textLength = this.__badgeText__.textContent.length;
                            var badgeWidth = Math.min(160, textLength * 7 + 30);
                            this.__badgeRect__.setAttribute("width", badgeWidth);
                        }

                        // Update status dot position (dynamic based on node width)
                        if (this.__statusDot__) {
                            this.__statusDot__.setAttribute("cx", d.w - 12);
                            this.__statusDot__.setAttribute("cy", 8);
                        }

                        // Get badge info for type text
                        function getNodeBadgeInfo(nodeType) {
                            var badges = {
                                // Network & HTTP
                                'http request': { text: 'HTTP Request', color: '#059669', icon: '○' },  // emerald-600
                        ...
                        // Create type text (first row - uppercase service type)
                        var typeTextElement = this.__typeTextGroup__.querySelector('.red-ui-flow-node-type-text');
                        if (!typeTextElement) {
                            typeTextElement = document.createElementNS("http://www.w3.org/2000/svg","text");
                            //typeTextElement.setAttribute("class","red-ui-flow-node-type-text");
                            typeTextElement.setAttribute("x",0);
                            typeTextElement.setAttribute("y",0);
                            typeTextElement.setAttribute("font-family", "Inter, 'Segoe UI', system-ui, sans-serif");
                            typeTextElement.setAttribute("font-size","10px");
                            typeTextElement.setAttribute("font-weight","0.05em");
                            typeTextElement.setAttribute("letter-spacing", "0.05em");
                            typeTextElement.setAttribute("text-transform", "uppercase");
                            typeTextElement.style["text-rendering"] = "optimizeLegibility";
                            typeTextElement.style["-webkit-font-smoothing"] = "antialiased";
                            typeTextElement.style["-moz-osx-font-smoothing"] = "grayscale";
                            typeTextElement.style["fill"] = "var(--theme-node-text-secondary)";
                            this.__typeTextGroup__.appendChild(typeTextElement);
                        }
                        
                        // Set type text content based on node type
                        var typeText = badgeInfo.text.toUpperCase();
                        typeTextElement.textContent = typeText;

                        if (labelParts) {
                            // The label has changed - limit to 2 lines maximum
                            var sa = labelParts.lines.slice(0, 2); // Only take first 2 lines
                            var sn = sa.length;
                            var textLines = this.__textGroup__.childNodes;
                            while(textLines.length > sn) {
                                textLines[textLines.length-1].remove();
                            ...
                            for (var i=0; i<sn; i++) {
                                if (i===textLines.length) {
                                    ...
                                    line.setAttribute("class","red-ui-flow-node-label-text");
                                    line.setAttribute("x",0);
                                    line.setAttribute("y",i*16); // Tighter line spacing
                                    line.setAttribute("font-family", "Roboto, 'Segoe UI', system-ui, sans-serif");
                                    line.setAttribute("font-size","14"); // Larger font for better readability
                                    line.setAttribute("font-weight","600"); // Semibold for better clarity
                                    // Theme-based text color will be set via CSS class
                                    line.setAttribute("letter-spacing", "0.01em");
                                    line.style["text-rendering"] = "optimizeLegibility";
                                    line.style["-webkit-font-smoothing"] = "antialiased";
                                    line.style["-moz-osx-font-smoothing"] = "grayscale";
                                    // Enable SVG text wrapping if supported (SVG2)
                                    // For browsers that support it, use CSS for wrapping
                                    this.__textGroup__.appendChild(line);
                                }
                                textLines[i].textContent = sa[i];
                            }
                        ...
                        // Position type text (always visible) - vertically centered relative to node height
                        var nodeCenterY = d.h / 2;
                        var typeYp = nodeCenterY - 8; // Center vertically, offset up slightly for type text
                        this.__typeTextGroup__.setAttribute("transform", "translate(64," + typeYp + ")");

                        // Position name text based on whether we have a label
                        var yp;
                        if (hideLabel || (labelParts && labelParts.isEmpty)) {
                            // No label - hide name text completely by positioning it off-screen
                            yp = -1000;
                        } else {
                            // Has label - position below type text, centered relative to node height
                            yp = nodeCenterY + 8; // Center vertically, offset down slightly for name text
                        }
                        ...
                        this.__textGroup__.classList.remove("red-ui-flow-node-label-right");
                        this.__textGroup__.setAttribute("transform", "translate(64," + yp + ")");

                        var inputPortGroup = this.__inputPortGroup__;
                        if ((!isLink || (showAllLinkPorts === -1 && !activeLinkNodes[d.id])) && d.inputs === 0) {
                            // Hide input port
                            inputPortGroup.style.display = "none";
                            if (inputPortGroup.__port__) {
                                ...
                                inputPortGroup.__port__ = null;
                            }
                        } else if (((isLink && (showAllLinkPorts===PORT_TYPE_INPUT||activeLinkNodes[d.id]))|| d.inputs === 1)) {
                            // Show and configure input port
                            inputPortGroup.style.display = "";
                            
                            if (!inputPortGroup.__port__) {
                                // Create the port element if it doesn't exist
                                var inputGroupPorts;
                                if (d.type === "link in") {
                                    inputGroupPorts = document.createElementNS("http://www.w3.org/2000/svg","circle");
                                    inputGroupPorts.setAttribute("cx",-1);
                                    inputGroupPorts.setAttribute("cy",5);
                                    inputGroupPorts.setAttribute("r",5);
                                    inputGroupPorts.setAttribute("class","red-ui-flow-port red-ui-flow-link-port");
                                } else {
                                    inputGroupPorts = document.createElementNS("http://www.w3.org/2000/svg","rect");
                                    inputGroupPorts.setAttribute("class","red-ui-flow-port");
                                    inputGroupPorts.setAttribute("rx",5);
                                    inputGroupPorts.setAttribute("ry",5);
                                    inputGroupPorts.setAttribute("width",10);
                                    inputGroupPorts.setAttribute("height",10);
                                }
                                
                                inputPortGroup.appendChild(inputGroupPorts);
                                inputPortGroup.__port__ = inputGroupPorts;
                                inputGroupPorts.__data__ = this.__data__;
                                inputGroupPorts.__portType__ = PORT_TYPE_INPUT;
                                inputGroupPorts.__portIndex__ = 0;
                                
                                inputGroupPorts.addEventListener("mousedown", portMouseDownProxy);
                                inputGroupPorts.addEventListener("touchstart", portTouchStartProxy);
                                inputGroupPorts.addEventListener("mouseup", portMouseUpProxy);
                                inputGroupPorts.addEventListener("touchend", portTouchEndProxy);
                                inputGroupPorts.addEventListener("mouseover", portMouseOverProxy);
                                inputGroupPorts.addEventListener("mouseout", portMouseOutProxy);
                                
                                RED.hooks.trigger("viewAddPort",{node:d,el: this, port: inputPortGroup, portType: "input", portIndex: 0});
                            }
                        }
                        ...
                        for(var portIndex = 0; portIndex < numOutputs; portIndex++ ) {
                            ...
                            if (portIndex === this.__outputs__.length) {
                                ...
                                if (d.type === "link out") {
                                    ...
                                    portPort.setAttribute("rx",5);
                                    portPort.setAttribute("ry",5);
                                ...
                                this.__outputPortsContainer__.appendChild(portGroup);
                        ...
                        if (d._def.icon) {
                            ...
                            this.__iconShade__.setAttribute("d", hideLabel ?
                                `M5 0 h${iconShadeWidth-10} a 5 5 0 0 1 5 5 v${iconShadeHeight-10} a 5 5 0 0 1 -5 5 h-${iconShadeWidth-10} a 5 5 0 0 1 -5 -5` : (
                                    "right" === d._def.align ?
                            ...
                            // Skip icon border updates for clean card design
                            // this.__iconShadeBorder__.style.display = hideLabel?'none':''
                            // this.__iconShadeBorder__.setAttribute("d", "M " + (((!d._def.align && d.inputs !== 0 && d.outputs === 0) || "right" === d._def.align) ? 0.5 : 29.5) + " "+(d.selected?1:0.5)+" l 0 " + (d.h - (d.selected?2:1)));
                        ...
                        if (this.__inputPortGroup__ && this.__inputPortGroup__.style.display !== "none") {
                            this.__inputPortGroup__.setAttribute("transform", "translate(-5,"+((d.h/2)-5)+")");
                        }
                        ...
                        if (d._def.button) {
                            ...
                            if (d._def.button.toggle) {
                                var isActive = d[d._def.button.toggle];
                                
                                // For modern circular buttons (inject and debug)
                                if (d.type === "inject" || d.type === "debug") {
                                    this.__buttonGroup__.classList.toggle("red-ui-flow-node-button-disabled", !isActive);
                                    
                                    // Update debug node icon based on state
                                    if (d.type === "debug" && this.__buttonGroupIcon__) {
                                        this.__buttonGroupIcon__.textContent = isActive ? "\uf06e" : "\uf070"; // eye or eye-slash
                                    }
                                } else {
                                    // Legacy rectangular button opacity
                                    this.__buttonGroupButton__.setAttribute("fill-opacity", isActive ? 1 : 0.2);
                                    this.__buttonGroupBackground__.setAttribute("fill-opacity", isActive ? 1 : 0.2);
                                }
                            }
                ...
                if (movingSet.length() > 0) {
                    ...
                    for (i=0;i<l;i++) {
                        ...
                        if (node.n.type !== 'junction') {
                            // Calculate dynamic dimensions
                            var dimensions = calculateNodeDimensions(node.n);
                            node.n.w = dimensions[0];
                            node.n.h = dimensions[1];
                            node.n.resize = true;
                        }
    ...
    function calculateNodeDimensions(node) {
        ...
        try {
            ...
            // Get the actual label that will be displayed
            var label = RED.utils.getNodeLabel(node, "");
            
            // For certain node types, ensure we get the full dynamic content
            if (!label || label.trim() === "" || label === node.type) {
                // Fallback to constructing the label based on node type
                if (node.type === "api-endpoint" && node.method && node.url) {
                    label = `${node.method} ${node.url}`;
                } else if (node.type === "http in" && node.method && node.url) {
                    label = `[${node.method}] ${node.url}`;
                } else if (node.type === "http request" && node.method && node.url) {
                    label = `${node.method} ${node.url}`;
                } else if (node.type === "typescript-function" && node.func) {
                    // For TypeScript function nodes, try to get a meaningful name
                    var funcName = node.func.match(/function\s+(\w+)/);
                    if (funcName) {
                        label = funcName[1];
                    } else if (node.name) {
                        label = node.name;
            ...
            // First pass: calculate dimensions with minimum width to get proper text wrapping
            var labelParts = getLabelParts(label, "red-ui-flow-node-label", node_width - 16);
            var originalLineCount = labelParts.lines.length; // Preserve original line count for height calculation
            
            // Calculate dynamic width based on content
            var calculatedWidth = node_width; // Start with minimum width
            
            if (!hideLabel && !labelParts.isEmpty && labelParts.width > 0) {
                // For better sizing, add generous padding to account for icon and margins
                var iconPadding = 72; // Space for icon (64px) + some margin
                var textMargin = 40; // Increased margin for more breathing room around text
                var textBasedWidth = labelParts.width + iconPadding + textMargin;
                var maxWidth = 500; // Increased maximum width for longer labels
                calculatedWidth = Math.min(Math.max(node_width, textBasedWidth), maxWidth);
                
                // Recalculate label parts with the new width to ensure proper wrapping (for display only)
                labelParts = getLabelParts(label, "red-ui-flow-node-label", calculatedWidth - iconPadding);
            }
            
            result[0] = calculatedWidth;
            
            // Dynamic height based on content (reduced by 10px from original)
            var baseHeight = 22; // Reduced from 32 to 22 (10px less)
            
            if (hideLabel || labelParts.isEmpty) {
                // No label needed - just show icon and type text
                result[1] = baseHeight + 8; // Icon + minimal padding
            } else {
                // Has label - add space for icon, type text and name text with proper line height
                var lineHeight = 16; // Spacing between lines
                var textPadding = 8; // Reduced padding for horizontal layout
                result[1] = baseHeight + textPadding + 16 + (originalLineCount * lineHeight); // Use original line count for consistent height
            }
            
            // Ensure minimum height for usability (reduced by 10px)
            result[1] = Math.max(result[1], 30);
        } catch(err) {
            console.log("Error calculating dimensions for node:", node);
            // Fallback on error (reduced by 10px)
            result = [node_width, 40];
        }
        return result;
    }
