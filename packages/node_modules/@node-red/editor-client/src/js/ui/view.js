...
RED.view = (function() {
    ...
    function init() {
        ...
        // Function to update pattern colors based on theme
        function updateDotPattern() {
            const isDark = document.body.getAttribute('data-theme') === 'dark';
            const bgColor = isDark ? '#111827' : '#f8fafc';
            const dotColor = isDark ? '#6b7280' : '#9ca3af'; // Higher contrast colors
        ...
        RED.events.on('theme:changed', updateDotPattern);
    ...
    function _redraw() {
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            nodeEnter.each(function(d,i) {
                ...
                badgeRect.__data__ = d;
                badgeRect.setAttribute("class", "red-ui-flow-node-badge");
                badgeRect.setAttribute("x", 8);
                badgeRect.setAttribute("y", 8);
                badgeRect.setAttribute("width", Math.min(160, badgeInfo.text.length * 7 + 30)); // Dynamic width based on text
                badgeRect.setAttribute("height", 20);
                badgeRect.setAttribute("rx", 3); // Less rounded corners (Flowbite style)
                badgeRect.setAttribute("ry", 3);
                badgeRect.setAttribute("fill", badgeInfo.color);
                badgeRect.setAttribute("stroke", "none"); // No border
                badgeRect.style["pointer-events"] = "none";
                badgeRect.style["fill"] = badgeInfo.color; // Ensure CSS doesn't override
                node[0][0].__badgeRect__ = badgeRect;
                nodeContents.appendChild(badgeRect);
                ...
                // Badge icon (Lucide icons only)
                var badgeIcon = createLucideIcon(d.type, 16, 18);
                badgeIcon.__data__ = d;
                badgeIcon.setAttribute("class", "red-ui-flow-node-badge-icon");
                badgeIcon.style["pointer-events"] = "none";
                node[0][0].__badgeIcon__ = badgeIcon;
                nodeContents.appendChild(badgeIcon);

                // Badge text - use EXACT same approach as crisp node label text
                ...
                // 5. NODE NAME TEXT (left-aligned under the badge)
                var text = document.createElementNS("http://www.w3.org/2000/svg","g");
                text.setAttribute("class","red-ui-flow-node-label"+(hideLabel?" hide":""));
                text.setAttribute("transform","translate(8, 42)"); // Left-aligned with balanced spacing from badge
                ...
                if (d.dirty) {
                    ...
                    if (d.resize || this.__hideLabel__ !== hideLabel || this.__label__ !== label || this.__outputs__.length !== d.outputs) {
                        ...
                        if (hideLabel || labelParts.isEmpty) {
                            ...
                            var lineHeight = 16; // Spacing between lines
                            var textPadding = 30; // Increased from 20 to 30 for better spacing
                            d.h = baseHeight + textPadding + (labelParts.lines.length * lineHeight);
                        }
                    ...
                    if (mouse_mode != RED.state.MOVING_ACTIVE) {
                        ...
                        if (labelParts) {
                            ...
                            for (var i=0; i<sn; i++) {
                                if (i===textLines.length) {
                                    ...
                                    line.setAttribute("font-size","15"); // Larger font for better readability
                        ...
                        // Position text based on whether we have a label and node height
                        var yp;
                        if (hideLabel || (labelParts && labelParts.isEmpty)) {
                            // No label - hide text completely by positioning it off-screen
                            yp = -1000;
                        } else {
                            // Has label - position below badge with more spacing
                            yp = 45; // Increased spacing from 35 to 45 for better separation
                        }
                        ...
                        this.__textGroup__.classList.remove("red-ui-flow-node-label-right");
                        this.__textGroup__.setAttribute("transform", "translate(8," + yp + ")");
    ...
    function calculateNodeDimensions(node) {
        ...
        try {
            ...
            if (hideLabel || labelParts.isEmpty) {
                ...
                var lineHeight = 16; // Spacing between lines
                var textPadding = 30; // Increased from 20 to 30 for better spacing
                result[1] = baseHeight + textPadding + (labelParts.lines.length * lineHeight);
            }
