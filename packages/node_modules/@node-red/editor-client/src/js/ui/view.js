            ...
            drop: function( event, ui ) {
                ...
                try {
                    ...
                    try {
                        ...
                        var baseHeight = 30; // Badge row height
                        
                        if (hideLabel || labelParts.isEmpty) {
                            // No label needed - just show badge row
                            nn.h = baseHeight;
                        } else {
                            // Has label - add space for text with proper line height
                            var lineHeight = 16; // Spacing between lines
                            var textPadding = 30; // Increased from 20 to 30 for better spacing
                            nn.h = baseHeight + textPadding + (labelParts.lines.length * lineHeight);
                        }
                        
                        // Ensure minimum height for usability
                        nn.h = Math.max(nn.h, 30);
                    } catch(err) {
                        // Fallback on error
                        nn.h = 50;
                        nn.w = node_width;
    ...
    function _redraw() {
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            if (activeSubflow) {
                ...
                subflowOutputs.each(function(d,i) {
                    if (d.dirty) {
                        ...
                        if (d.resize) {
                            var ow = d.w;
                            // Apply consistent width to subflow ports
                            d.w = node_width;
                            if (ow !== undefined) {
                                d.x += (d.w-ow)/2;
                            }
            ...
            nodeEnter.each(function(d,i) {
                ...
                // Icon background (larger colored square)
                ...
                // Node icon (Lucide icons) - centered in the icon rectangle
                var nodeIcon = createLucideIcon(d.type, 16, 16);
                nodeIcon.__data__ = d;
                ...
                statusDot.setAttribute("cx", node_width - 12);
            ...
            node.each(function(d,i) {
                ...
                if (d.dirty) {
                    ...
                    if (d.resize || this.__hideLabel__ !== hideLabel || this.__label__ !== label || this.__outputs__.length !== d.outputs) {
                        labelParts = getLabelParts(label, "red-ui-flow-node-label", node_width - 16);
                        if (labelParts.lines.length !== this.__labelLineCount__ || this.__label__ !== label) {
                            d.resize = true;
                        }
                        this.__label__ = label;
                        ...
                        // Dynamic height based on content  
                        var baseHeight = 32; // Larger icon height
                    ...
                    if (d.resize) {
                        var ow = d.w;
                        // Apply consistent width to all nodes
                        d.w = node_width;
                        if (ow !== undefined) {
                            d.x += (d.w-ow)/2;
                        }
                    ...
                    if (mouse_mode != RED.state.MOVING_ACTIVE) {
                        ...
                        // Update status dot position (static since width is consistent)
                        if (this.__statusDot__) {
                            this.__statusDot__.setAttribute("cx", node_width - 12);
                        ...
                        // Position type text (always visible)
                        var typeYp = 26; // Positioned lower for better visual balance
                        ...
                        if (hideLabel || (labelParts && labelParts.isEmpty)) {
                            ...
                            // Has label - position below type text
                            yp = 42; // Below type text, after larger icon with proper spacing
                        }
                ...
                if (movingSet.length() > 0) {
                    ...
                    for (i=0;i<l;i++) {
                        ...
                        if (node.n.type !== 'junction') {
                                                    node.n.w = node_width;
                        // Calculate minimal height - compact design
                        node.n.h = 50; // Compact height
                            node.n.resize = true;
                        }
    ...
    function calculateNodeDimensions(node) {
        ...
        try {
            ...
            if (hideLabel || labelParts.isEmpty) {
                ...
                var lineHeight = 16; // Spacing between lines
                var textPadding = 8; // Reduced padding for horizontal layout
                result[1] = baseHeight + textPadding + 16 + (labelParts.lines.length * lineHeight); // Icon + type + name
            }
            
            // Ensure minimum height for usability
            result[1] = Math.max(result[1], 40);
        } catch(err) {
