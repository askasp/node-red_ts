...
RED.deploy = (function() {
    ...
    function changeDeploymentType(type) {
        ...
        $("#canvas-deploy-button-icon").attr("src",deploymentTypes[type].img);
    }

    function initCanvasDeploy() {
        // Create deploy button container
        const deployContainer = $(`
            <div id="canvas-deploy-container" class="canvas-deploy-container">
                <div class="canvas-deploy-button-group">
                    <button id="canvas-deploy-button" class="canvas-deploy-button disabled">
                        <span class="canvas-deploy-button-content">
                            <img id="canvas-deploy-button-icon" src="red/images/deploy-full-o.svg">
                            <span>Deploy</span>
                        </span>
                        <span class="canvas-deploy-button-spinner hide">
                            <img src="red/images/spin.svg"/>
                        </span>
                    </button>
                    <button id="canvas-deploy-button-options" class="canvas-deploy-button-options">
                        <i class="fa fa-caret-down"></i>
                    </button>
                    <button id="canvas-options-button" class="canvas-options-button" title="Options">
                        <i class="fa fa-cog"></i>
                    </button>
                    <button id="canvas-ai-button" class="canvas-options-button" title="AI Assistant">
        ...
        // Append to workspace
        $("#red-ui-workspace").append(deployContainer);
        ...
        RED.menu.init({id:"canvas-deploy-button-options", options: mainMenuItems });
        
        // Setup options menu
        const optionsMenuItems = [
            {id:"options-menu-palette",label:"Show Palette",toggle:true,onselect:function(s) { 
                if (s) {
            ...
            {id:"options-menu-sidebar",label:"Show Sidebar",toggle:true,onselect:function(s) { 
                if (s) {
                    RED.actions.invoke("core:toggle-sidebar", true);
                } else {
                    RED.actions.invoke("core:toggle-sidebar", false);
                }
            }, selected: true},
            null,
            {id:"options-menu-project-color",label:"Project Color",onselect:function() { 
                // Redirect to proper project settings instead of localStorage-based color picker
                if (RED.projects && RED.projects.settings) {
                    RED.projects.settings.show('main');
                } else {
                    RED.notify("Projects must be enabled to use project colors", "warning");
                }
            }},
            {id:"options-menu-user-settings",label:"User Settings",onselect:function() { 
                RED.actions.invoke("core:show-user-settings");
            }},
            {id:"options-menu-help",label:"Help",href:"https://nodered.org/docs"}
        ];
        
        RED.menu.init({id:"canvas-options-button", options: optionsMenuItems });
        
        // Apply saved project color on startup
        setTimeout(function() {
            // Remove any old localStorage color settings to prevent conflicts
            localStorage.removeItem('node-red-project-color');
            localStorage.removeItem('node-red-project-theme');
            localStorage.removeItem('node-red-interface-color');
            
            console.log('Startup - localStorage color settings cleared, using project colors only');
        }, 500);
        
        // Update menu states based on current visibility
        RED.menu.setSelected("options-menu-palette", !$("#red-ui-main-container").hasClass("red-ui-palette-closed"));
        RED.menu.setSelected("options-menu-sidebar", !$("#red-ui-main-container").hasClass("red-ui-sidebar-closed"));
        
        // Listen for palette and sidebar state changes
        RED.events.on("sidebar:resize", function() {
            RED.menu.setSelected("options-menu-sidebar", !$("#red-ui-main-container").hasClass("red-ui-sidebar-closed"));
        });
        
        // Listen for palette state changes
        $(document).on("palette:resize", function() {
            RED.menu.setSelected("options-menu-palette", !$("#red-ui-main-container").hasClass("red-ui-palette-closed"));
        });
        
        // Setup event handlers
        $('#canvas-deploy-button').on("click", function(event) {
        ...
        style.text(`
            ...
            .canvas-deploy-button-group {
                display: flex;
                align-items: center;
                background: var(--theme-button-bg, #ffffff);
                border: 1px solid var(--theme-header-border, #e5e7eb);
                border-radius: 8px;
                box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06);
                overflow: hidden;
            }
            
            .canvas-deploy-button {
            ...
            .canvas-deploy-button:hover:not(.disabled) {
                background: var(--red-ui-deploy-button-background-hover);
                transform: translateY(-1px);
                box-shadow: 0 6px 8px -1px rgba(0, 0, 0, 0.1), 0 4px 6px -1px rgba(0, 0, 0, 0.06);
            }
            
            .canvas-deploy-button:focus {
                outline: none;
                box-shadow: 0 0 0 3px rgba(0, 0, 0, 0.1);
            }
            
            .canvas-deploy-button.disabled {
                cursor: not-allowed;
                background: #9ca3af;
                color: #6b7280;
                transform: none;
            ...
            .canvas-deploy-button.disabled:hover {
                background: #9ca3af;
                transform: none;
                box-shadow: none;
            }
            
            .canvas-deploy-button-content {
                display: flex;
                align-items: center;
                gap: 6px;
            }
            
            .canvas-deploy-button-content > img {
                height: 14px;
                width: 14px;
                opacity: 0.9;
            }
            
            .canvas-deploy-button.disabled .canvas-deploy-button-content > img {
                opacity: 0.5;
            }
            
            .canvas-deploy-button-spinner {
                position: absolute;
                left: 0;
                top: 0;
                right: 0;
            ...
            .canvas-deploy-button-spinner img {
                opacity: 0.8;
                height: 16px;
                width: 16px;
            }
            
            .canvas-deploy-button-options {
                background: var(--theme-button-bg, #f9fafb);
                border: none;
                border-left: 1px solid var(--theme-button-border, #e5e7eb);
                padding: 8px 12px;
                cursor: pointer;
            ...
            .canvas-deploy-button-options:hover {
                background: var(--theme-button-hover-bg, #f3f4f6);
                color: var(--theme-button-hover-text, #374151);
            }
            
            .canvas-deploy-button-options i {
                font-size: 12px;
            }
            
            .canvas-deploy-button-group.open .canvas-deploy-button-options {
                background: #e5e7eb;
                color: #374151;
            }
            
            .canvas-options-button {
                background: var(--theme-button-bg, #f9fafb) !important;
                border: none !important;
                border-left: 1px solid var(--theme-button-border, #e5e7eb) !important;
                padding: 8px 12px !important;
                cursor: pointer;
                color: var(--theme-button-text, #6b7280) !important;
                transition: all 0.2s ease;
                display: flex;
                align-items: center;
                justify-content: center;
            ...
            .canvas-options-button:hover {
                background: var(--theme-button-hover-bg, #f3f4f6) !important;
                color: var(--theme-button-hover-text, #374151) !important;
            }
            
            .canvas-options-button:focus {
                outline: none !important;
                background: var(--theme-button-hover-bg, #f3f4f6) !important;
                box-shadow: none !important;
            }
            ...
            .canvas-ai-button:hover {
                background: var(--theme-button-hover-bg, #f3f4f6) !important;
                color: #5a67d8 !important;
            }
            
            .canvas-ai-button:focus {
                outline: none !important;
                background: var(--theme-button-hover-bg, #f3f4f6) !important;
                box-shadow: none !important;
            }
    ...
    function init(options) {
        ...
        RED.events.on('workspace:dirty',function(state) {
            ...
            if (state.dirty) {
                ...
                // Also update canvas deploy button
                $("#canvas-deploy-button").removeClass("disabled");
            } else {
                // window.onbeforeunload = null;
                $("#red-ui-header-button-deploy").addClass("disabled");
                // Also update canvas deploy button
                $("#canvas-deploy-button").addClass("disabled");
            }
        });

        // Initialize canvas deploy button
        initCanvasDeploy();
    ...
    function deployButtonSetBusy(){
        $("#red-ui-header-button-deploy-spinner").removeClass("hide");
        $("#red-ui-header-button-deploy").addClass("disabled");
        // Also update canvas deploy button
        $("#canvas-deploy-button-spinner").removeClass("hide");
        $("#canvas-deploy-button").addClass("disabled");
    }
    
    function deployButtonClearBusy(){
        $("#red-ui-header-button-deploy-spinner").addClass("hide");
        $("#red-ui-header-button-deploy").removeClass("disabled");
        // Also update canvas deploy button
        $("#canvas-deploy-button-spinner").addClass("hide");
        $("#canvas-deploy-button").removeClass("disabled");
    }
    ...
    function save(skipValidation, force) {
        ...
        $.ajax({
            ...
            // Emit deploy complete event
            RED.events.emit('deploy:complete');
        }).fail(function (xhr, textStatus, err) {
