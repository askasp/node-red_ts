...
RED.menu = (function() {
    ...
    function createMenuItem(opt) {
        console.log("=== createMenuItem called with ===", opt);
        var item;

        if (opt !== null && opt.id) {
            var themeSetting = RED.settings.theme("menu."+opt.id);
            if (themeSetting === false) {
                console.log("Menu item hidden by theme setting:", opt.id);
                return null;
            }
        }
        ...
        if (opt === null) {
            ...
            if (!opt.id) {
                ...
                console.log("Generated menu item ID:", opt.id, "for label:", opt.label);
            }
            if (opt.group) {
            ...
            if (opt.options) {
                ...
                // Enhanced submenu positioning and event handling
                submenu.css({
                    'position': 'absolute',
                    'display': 'none',
                    'z-index': '1002',
                ...
                item.on('mouseenter', function() {
                    ...
                    $('.red-ui-menu-dropdown', item.parent()).not(submenu).hide();
                    $('.red-ui-menu-dropdown-submenu', item.parent()).removeClass('open');
                    
                    // Position submenu correctly
                    var itemOffset = item.position();
                    var parentMenu = item.closest('.red-ui-menu-dropdown');
                    var windowWidth = $(window).width();
                    var submenuWidth = submenu.outerWidth() || 200;
                    var parentLeft = parentMenu.offset().left;
                    var parentRight = parentLeft + parentMenu.outerWidth();
                    
                    // For pull-left menus or when there's not enough space on the right
                    if (item.hasClass('pull-left') || parentRight + submenuWidth > windowWidth - 20) {
                        // Position submenu to the left of parent, but not too far
                        var leftPos = Math.min(submenuWidth, 220);
                        submenu.css({
                            'left': '-' + leftPos + 'px',
                            'right': 'auto'
                        });
                ...
                // Handle clicks properly - prevent propagation issues
                link.on('click', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    
                    // For touch devices or explicit clicks, toggle submenu
