...
RED.utils = (function() {
    ...
    function getLighterColor(c, lightness) {
        // Parse the color
        var r,g,b;
        if (/^#[a-f0-9]{6}$/i.test(c)) {
            r = parseInt(c.substring(1, 3), 16);
        ...
        // Create a very light version for canvas (similar to Arc's approach)
        // Use high lightness (92-97%) with reduced saturation for pleasant background
        var canvasLightness = lightness || 0.95;
        var canvasSaturation = Math.min(s * 0.15, 0.08); // Very desaturated
        
        // Convert back to RGB
        var c1 = (1 - Math.abs(2 * canvasLightness - 1)) * canvasSaturation;
        var x = c1 * (1 - Math.abs(((h * 6) % 2) - 1));
        var m = canvasLightness - c1 / 2;
        
        var rNew, gNew, bNew;
        if (h < 1/6) {
            rNew = c1; gNew = x; bNew = 0;
        } else if (h < 2/6) {
            rNew = x; gNew = c1; bNew = 0;
        } else if (h < 3/6) {
            rNew = 0; gNew = c1; bNew = x;
        } else if (h < 4/6) {
            rNew = 0; gNew = x; bNew = c1;
        } else if (h < 5/6) {
            rNew = x; gNew = 0; bNew = c1;
        } else {
            rNew = c1; gNew = 0; bNew = x;
        }
        ...
        var result = ((rNew<<16) + (gNew<<8) + bNew).toString(16);
